// Simple on-screen joystick for touch devices. Writes a normalized -1..1
// vector into `output` (a THREE.Vector2) each frame so PlayerController can
// merge it with keyboard input. Only shown on touch-capable devices — see
// the isTouchDevice check below. Positioning (bottom-center on phones,
// bottom-left on tablets) is handled entirely by CSS media queries on
// #joystick-base; this class doesn't need to know which layout is active.
export class TouchJoystick {
  constructor(output) {
    ...
    window.addEventListener('touchcancel', (e) => this._end(e));
  }

  _start(e) {
    e.preventDefault();
    this.active = true;
    // Re-measure on every press rather than once at construction — the base
    // can move between a phone and tablet layout (or on resize/rotation).
